...
void base_dds_initializer(void)
{
  bytes_expected = 0;
...
void dds_initializer(DeviceObject *o)
{
  ...
  dev_dds_data *data = malloc(sizeof(dev_dds_data));
  if (cfg)
  {
...
static int exec_command(dev_dds *config, DeviceObject *o, unsigned char *buffer)
{
  dds_cmd command;
  int rc = 1;
  dev_dds_data *data = (dev_dds_data *)o->device_data;
  ...
  switch (last_command)
  {
    ...
    case 'c': // set frequency code
      ...
      rc = config->command(o, last_command == 'f' ? DDS_COMMAND_SET_FREQUENCY : DDS_COMMAND_SET_FREQUENCY_CODE,
                            &command);
#else
      rc = DDS_COMMAND(o, last_command == 'f' ? DDS_COMMAND_SET_FREQUENCY : DDS_COMMAND_SET_FREQUENCY_CODE,
                            &command);
#endif      
      break;
    case 'm': // set mode
...
#else
      rc = DDS_COMMAND(o, DDS_COMMAND_SET_MODE, &command);
#endif      
      break;
...
#else
      rc = DDS_COMMAND(o, DDS_COMMAND_SET_ATTENUATOR, &command);
#endif      
      break;
...
#else
      rc = DDS_COMMAND(o, DDS_COMMAND_ENABLE_OUTPUT, &command);
#endif      
      break;
    case 's': // sweep
    case 'd': // sweep codes
      ...
      data->sweep_points = command.sweep_command.points;
#ifndef DDS_COMMAND
      rc = config->command(o, last_command == 's' ? DDS_COMMAND_SWEEP : DDS_COMMAND_SWEEP_CODES,
                            &command);
#else
      rc = DDS_COMMAND(o, last_command == 's' ? DDS_COMMAND_SWEEP : DDS_COMMAND_SWEEP_CODES,
                            &command);
#endif      
      break;
    default:
...
int dds_timer_event(DeviceObject *o, int step, unsigned char *buffer)
{
  ...
  dev_dds_data *data = (dev_dds_data *)o->device_data;
  if (cfg->cfg.level_meter_type != LEVEL_METER_NONE && data->sweep_points > 0 && get_interrupt_pin_level(o))
  {
    unsigned char c = DEVICE_COMMAND_GET_RESULTS;
    unsigned int size = data->sweep_points * 2;
