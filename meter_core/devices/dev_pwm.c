...
#ifndef DEV_PWM_SET_FREQUENCY_AND_DUTY      
      rc = dev->set_frequency_and_duty(o, channel, prescaler, frequency, duty) ? 3 : 0;
#else
      rc = DEV_PWM_SET_FREQUENCY_AND_DUTY(o, channel, prescaler, frequency, duty) ? 3 : 0;
#endif      
      break;
    case 'e': // enable output
#ifndef DEV_PWM_ENABLE_OUTPUT
      rc = dev->enable_output(o, channel, *command_buffer_p) ? 2 : 0;
#else
      rc = DEV_PWM_ENABLE_OUTPUT(o, channel, *command_buffer_p) ? 2 : 0;
#endif      
      break;
    default:
      break;
...
static int external_pwm_set_frequency_and_duty(DeviceObject *o, int channel, unsigned short prescaler,
                                                unsigned long frequency, unsigned long duty)
{
  pwm_i2c_command cmd;
